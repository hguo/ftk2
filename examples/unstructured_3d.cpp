 ...
 * 
 * Demonstrates 3D steady-state feature extraction (contour, fiber, critical point)
...
int main(int argc, char** argv) {
    ...
    std::cout << "Mesh has " << n_verts << " vertices." << std::endl;
    ...
    for (int i = 0; i < n_verts; ++i) {
        ...
        // Midpoint of [0, 75] is 37.5. Offset slightly to avoid exact vertex alignment.
        double cx = 37.6, cy = 37.7, cz = 37.8;
        double dx = x - cx, dy = y - cy, dz = z - cz;

        // 1. Scalar field for contour: Sphere centered at (cx, cy, cz)
        s[i] = std::sqrt(dx*dx + dy*dy + dz*dz) - 15.0; // r=15

        // 2. Vector field for CP and Fiber tracking:
        u[i] = dx;
        v[i] = dy;
        w[i] = dz;
    }
    ...
    // --- 1. Extract Contour (m=1 in 3D space -> 2D manifold) ---
    {
        std::cout << "Extracting Contour (surface)..." << std::endl;
        ContourPredicate<double> pred; pred.var_name = "S";
        SimplicialEngine<double, ContourPredicate<double>> engine(mesh, pred);
        engine.execute(data);
        auto complex = engine.get_complex();
        std::cout << "  Found " << complex.vertices.size() << " nodes on contour surface." << std::endl;
        write_complex_to_vtu(complex, *mesh, "unstructured_3d_contour.vtu", 2);
    }

    // --- 2. Extract Fiber (m=2 in 3D space -> 1D manifold) ---
    {
        ...
        engine.execute(data);
        auto complex = engine.get_complex();
        std::cout << "  Found " << complex.vertices.size() << " nodes on fiber curve." << std::endl;
        write_complex_to_vtu(complex, *mesh, "unstructured_3d_fiber.vtu", 1);
    }

    // --- 3. Extract Critical Point (m=3 in 3D space -> 0D manifold) ---
    {
        std::cout << "Extracting Critical Point (vertex)..." << std::endl;
        ...
        // Output as 0D vertex cells so they are visible
