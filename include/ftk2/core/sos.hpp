...
public:
    ...
    static bool check(const T v[4][3], const uint64_t idx[4], double Q = 1000000.0) {
        int s[4];
        s[0] = det3(v[0], v[1], v[2], idx[0], idx[1], idx[2], Q);
        s[1] = det3(v[1], v[2], v[3], idx[1], idx[2], idx[3], Q);
        s[2] = det3(v[2], v[3], v[0], idx[2], idx[3], idx[0], Q);
        s[3] = det3(v[3], v[0], v[1], idx[3], idx[0], idx[1], Q);
