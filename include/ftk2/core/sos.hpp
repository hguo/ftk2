...
/**
 * @brief Quantization factor for robust predicates.
 */
static constexpr double SoS_Q = 1000000.0;

/**
 * @brief Robust sign check using quantized Simulation of Simplicity.
 */
template <typename T>
FTK_HOST_DEVICE
inline int sign(T v, uint64_t index) {
    long long qv = static_cast<long long>(std::round(v * SoS_Q));
    if (qv > 0) return 1;
    if (qv < 0) return -1;
    return 1; 
}
...
/**
 * @brief Robust 2x2 determinant sign using quantized SoS.
 */
template <typename T>
FTK_HOST_DEVICE
inline int det2(const T v0[2], const T v1[2], uint64_t i0, uint64_t i1) {
    auto q = [](T val) { return static_cast<__int128>(std::round(val * SoS_Q)); };
...
/**
 * @brief Robust 3x3 determinant sign using quantized SoS.
 */
template <typename T>
FTK_HOST_DEVICE
...
/**
 * @brief Predicate to check if the origin is inside a K-simplex.
 */
